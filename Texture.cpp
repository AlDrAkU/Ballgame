...
void Texture::CreateFromImage( const std::string& path )
{
	...
	{
		std::cout << "Texture::CreateFromImage, error when calling IMG_Load: " << SDL_GetError( ) << std::endl;
		m_CreationOk = false;
		return;
	}
...
void Texture::CreateFromString( const std::string& text, const std::string& fontPath, int ptSize, const Color4f& textColor )
{
	...
	if(pFont == nullptr )
	{
		std::cout << "Texture::CreateFromString, error when calling TTF_OpenFont: " << TTF_GetError( ) << std::endl;
		m_CreationOk = false;
		return;
	}
...
void Texture::CreateFromString( const std::string& text, TTF_Font *pFont, const Color4f& color )
{
	...
	if ( pLoadedSurface == nullptr )
	{
		std::cout << "Texture::CreateFromString, error when calling TTF_RenderText_Blended: " << TTF_GetError( ) << std::endl;
		m_CreationOk = false;
		return;
	}
...
void Texture::CreateFromSurface( SDL_Surface *pSurface )
{
	...
	case 4:
		...
		break;
	default:
		std::cout << "Texture::CreateFromSurface, unknow pixel format, BytesPerPixel: " << pSurface->format->BytesPerPixel << "\nUse 32 bit or 24 bit images.\n";
		m_CreationOk = false;
		return;
	}
...
void Texture::Draw( const Point2f& dstBottomLeft, const Rectf& srcRect )
{
	if ( !m_CreationOk )
	{
...
void Texture::Draw( const Rectf& destRect, const Rectf& srcRect )
{
	if ( !m_CreationOk )
	{
...
bool Texture::IsCreationOk( )
{
	return m_CreationOk;
}

void Texture::DrawFilledRect( const Point2f& dstBottomLeft )
{
